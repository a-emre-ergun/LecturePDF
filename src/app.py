...
from src.utils.platform import (
    is_android, is_desktop, PLATFORM,
    get_system_locale, get_app_data_dir, get_bundle_dir
)
...
from src.mixins.naming_mixin import NamingMixin

from src.ui.screens.create_pdfs import CreatePDFsScreen
from src.ui.screens.history import HistoryScreen
...
class App(ScheduleMixin, CourseMixin, HistoryMixin, PDFGenerationMixin, ThemeMixin, NamingMixin, MDApp):
    ...
    current_naming_format_name = StringProperty("Week First")
    ...
    dynamic_color_enabled = BooleanProperty(False)
    is_android_platform = BooleanProperty(False)

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        self.is_android_platform = is_android()
        ...
        self.index_file = os.path.join(
            self.app_data_dir, "schedules_index.json")
    ...
    def _resolve_language(self, saved_lang):
    ...
    def build(self):
        saved_lang = self.get_setting("language") or "en"
        resolved_lang = self._resolve_language(saved_lang)
        ...
        try:
            ...
            if not os.path.isdir(assets_path):
                assets_path = os.path.join(
                    os.path.dirname(self.bundle_dir), "assets")
            self.tr = Language(resolved_lang, language_path=assets_path)
            self._ = self.tr._
    ...
    def on_start(self):
        ...
        if not os.path.exists(icon_path):
            icon_path = os.path.join(os.path.dirname(
                self.bundle_dir), "assets", icon_name)
            ...
            if icon_name.endswith(".ico"):
                try:
                    ...
                    png_path = os.path.join(
                        tempfile.gettempdir(), "lecturepdf_icon.png")
        ...
        saved_dynamic = self.get_setting("dynamic_color")
        if saved_dynamic and self.is_android_platform:
            self.dynamic_color_enabled = True
        ...
        if not is_tutorial_done:
            Clock.schedule_once(self.start_tutorial, 2)
    ...
    def _unfocus_active_input(self):
        try:
    ...
    def handle_back_button(self, window, key, scancode, codepoint, modifier):
        if key != 27:
            return False

        if self._unfocus_active_input():
            return True

        if self._dismiss_open_dialog():
            return True

        current = self.root.ids.screen_manager.current
        if current in ("LanguageScreen", "NamingFormatScreen"):
            self.go_back_to_settings()
            return True
        elif current == "ScheduleDetail":
            self.go_back_to_schedules()
            return True
        elif current == "HistoryScreen":
            self.go_back_from_history()
            return True
        elif current in ("Schedules", "Settings"):
    ...
    def start_tutorial(self, dt):
        ...
        if is_desktop():
            ...
            steps = [
                ...
                (screen.ids.btn_generate_step,
                 self._("Step 3: Create PDF"),
                 self._("If everything is ready, tap here and create PDFs!")),

                (self.root.ids.history_btn,
                 self._("PDF History"),
                 self._("Tap here to view your previously generated PDFs.")),

                (self.root.ids.nav_item_schedules,
                 self._("Schedules"),
                 self._("Tap here to add schedules.")),

                (self.root.ids.nav_item_settings,
                 self._("Settings"),
                 self._("Tap here to change output folder."))
            ]
        ...
        for child in widget.walk():
            if hasattr(child, "text") and child.text:
                ...
                if not widget._headers_translated and len(original) == 1 and original in "MTWTFSS":
                    ...
                    import re
                    if re.search(r"\bMay\b", new_text):
                        may_full = self.tr._("May_full")
                        if may_full == "May_full":
                            may_full = "May"
                        new_text = re.sub(
                            r"\bMay(?=\s+\d{4})", may_full, new_text)
                        may_abbr = self.tr._("May")
                        new_text = re.sub(r"\bMay\b", may_abbr, new_text)
    ...
    def _translation_hints():
        ...
        _("File missing!")
        _("PDF Naming Format")
        _("Customize PDF file names")
        _("Format Name")
        _("Format String")
        _("Available variables: {course}, {week}, {schedule}, {num_photos}")
        _("Example: {course}_lecture_{week:02d}")
        _("Add Format")
        _("Edit Format")
        _("Delete Format?")
